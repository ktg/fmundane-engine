...
import { convertToTwine, renderSpeech, replaceAll } from '../utils';
...
authorRouter.get('/translate', (req, res) => {
	...
	try {
		const layers = JSON.parse(file.toString());
...
export async function render(name) {
	...
	const placeholderText = fs.readFileSync('../placeholders/placeholders.json');
	const placeholders = JSON.parse(placeholderText.toString());
	...
	for (const layer of results) {
		...
		for (const line of torender) {
			...
			const result = await request.get('http://localhost:5002/api/generate').query({
				t: message,
				v: _voice,
				n: name,
			});
